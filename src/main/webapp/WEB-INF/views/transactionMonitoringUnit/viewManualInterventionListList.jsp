        ...
        <div class="card-block">
            ...
            <form method="post" id="configureNumberOfDaysForm" action=""   >
                <div style="margin-left: 15px; margin-right: 15px;" class="alert alert-info">
                    Executed Manual Intervention Processes
                </div>
                ...
                <div class="dt-responsive table-responsive">
                    <table id="simpletable" class="table table-hover table-striped table-bordered nowrap" style="width:100%">
                        ...
                        <c:if test="${not empty apvRejList}">
                            <c:forEach var="apvRejObject" items="${apvRejList}" varStatus="counter">
                                <tr>
                                    <td>${counter.count}.</td>
                                    <td>${apvRejObject.processType}</td>
                                    <td>${apvRejObject.tpinType}</td>
                                    <td>${apvRejObject.tpin}</td>
                                    <td>${apvRejObject.status}</td>
                                    <%--<td>${apvRejObject.referenceNumber}</td>--%>
                                    <td class="text-center">
                                        <a  href="transactionMonitoringUnit/viewExecutedTaskManualInteView?refNumber=${apvRejObject.referenceNumber}" class="text-info">${apvRejObject.referenceNumber}

                                        </a>
                                    </td>
                                    <td>${apvRejObject.modifiedDate}</td>
